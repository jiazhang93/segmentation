...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
    ...
    # prob_thr = 0.2
    # print(f"prob thr: {prob_thr}")
    # print(seg_old_spacing[1,].shape)
    ...
    # seg_prob_mask = np.around(seg_old_spacing[0,], 6).astype(np.float16)
    # seg_prob_itk = sitk.GetImageFromArray(seg_prob_mask.astype(np.single))
    # seg_prob_itk.SetSpacing(properties_dict['itk_spacing'])
    # seg_prob_itk.SetOrigin(properties_dict['itk_origin'])
    # seg_prob_itk.SetDirection(properties_dict['itk_direction'])
    # sitk.WriteImage(seg_prob_itk, 
    # out_fname.replace(out_fname.split('/')[-1], 
    #                   out_fname.split('/')[-1].replace('.nii.gz', '_bgsoftmax.nii.gz')))

    # sitk.WriteImage(seg_old_spacing[1,].astype(np.float16), 
    #     out_fname.replace(out_fname.split('/')[-1], 'softmax_'+out_fname.split('/')[-1])
    # )
    ...
    if region_class_order is None:
        ...
        for i, c in enumerate(region_class_order):
            seg_old_spacing_final[seg_old_spacing[i] > prob_thr] = c
...
def save_segmentation_nifti(segmentation, out_fname, dct, order=1, force_separate_z=None, order_z=0, verbose: bool = False, probs_thr=0.5):
    """
