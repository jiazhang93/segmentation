...
class GenericPreprocessor(object):
    ...
    def resample_and_normalize(self, data, target_spacing, properties, seg=None, force_separate_z=None):
        ...
        # print('total c:', len(data))
        for c in range(len(data)):
            scheme = self.normalization_scheme_per_modality[c]
            # print('scheme:', scheme)

            # print('c:', c,'scheme:', self.normalization_scheme_per_modality[1])
            ...
            if scheme == "CT":
                ...
                if use_nonzero_mask[c]:
                    ...
                    print('use_nonzero_mask', use_nonzero_mask[c])
                    mn = data[c].mean()
                    std = data[c].std()
                    # print('data property:', c, data[c].shape, data[c].dtype, mn, std)
                    data[c] = (data[c] - mn) / (std + 1e-8)

                    # if c==0:
                    #     print(c,'use_nonzero_mask', use_nonzero_mask[c])
                    #     mn = data[c].mean()
                    #     std = data[c].std()
                    #     # print('data property:', c, data[c].shape, data[c].dtype, mn, std)
                    #     data[c] = (data[c] - mn) / (std + 1e-8)
                    ...
                    #     print('data property:', c, data[c].shape, data[c].dtype, mn, std, min_val)

                    #     data[c][data[c]<lower_bound] = min_val
                    #     data[c][data[c]>upper_bound] = min_val

                    #     data[c] = (data[c] - mn) / (std + 1e-8)

            # print('ct thred:', self.intensityproperties[c]['percentile_00_5'], 
            # self.intensityproperties[c]['percentile_99_5'])
