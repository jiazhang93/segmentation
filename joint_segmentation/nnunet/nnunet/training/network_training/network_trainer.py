...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.win_val_loss = None
        self.win_val_dice_score = None
        ...
        self.minloss_epoch = 0
        self.min_val_loss = 0

        self.maxsore_epoch = 0
        self.max_dice_sore = 0
    ...
    def initialize_visdom(self):
        ###################### visdom ##########################
        self.print_to_log_file("Setting up visdom")
        self.viz = Visdom(port=3666)
        self.win_train_loss = self.viz.line(X=np.array([0]), Y=np.array([0.0]), opts=dict(title='train_loss'))
        self.win_val_loss = self.viz.line(X=np.array([0]), Y=np.array([0.0]), opts=dict(title='val_loss'))
        self.win_val_dice_score = self.viz.line(X=np.array([0]), Y=np.array([0.0]), opts=dict(title='val dice score'))
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            if self.use_progress_bar:
                ...
                for _ in tqdm(range(self.num_batches_per_epoch)):
                    l = self.run_iteration(self.tr_gen, True)
                    train_losses_epoch.append(l)
            ...
            ################ add visdom show
            if self.viz:
                self.viz.line(X=np.array([self.epoch]), Y=np.array([self.all_tr_losses[-1]]), win=self.win_train_loss, update='append')
            ...
            with torch.no_grad():
                ...
                current_val_loss = self.all_val_losses[-1]
                if current_val_loss<self.min_val_loss:
                    self.minloss_epoch = self.epoch
                    self.min_val_loss = current_val_loss

                self.print_to_log_file("min validation loss: %.4f in %d epoch" % \
                    (self.min_val_loss, self.minloss_epoch))
                ############### add visdom show
                if self.viz:
                    self.viz.line(X=np.array([self.epoch]), Y=np.array([self.all_val_losses[-1]]), win=self.win_val_loss, update='append')
