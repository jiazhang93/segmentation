...
class nnUNetTrainer(NetworkTrainer):
    ...
    def initialize_visdom(self):
        super(nnUNetTrainer, self).initialize_visdom()
    ...
    def predict_preprocessed_data_return_seg_and_softmax(self, data: np.ndarray, do_mirroring: bool = True,
        ...
        self.network.eval()
        # print('network:', self.network.conv_op)
    ...
    def finish_online_evaluation(self):
        ...
        ############### add visdom show
        if self.viz:
            self.viz.line(X=np.array([self.epoch]), Y=np.array([self.all_val_eval_metrics[-1]]), 
                      win=self.win_val_dice_score, update='append')

        current_dice_sore = self.all_val_eval_metrics[-1]
        if current_dice_sore>self.max_dice_sore:
            self.max_dice_sore = current_dice_sore
            self.maxsore_epoch = self.epoch

        self.print_to_log_file(f"(Not all val set)Best global foreground Dice :{self.max_dice_sore} in {self.maxsore_epoch} epoch")
