...
class TverskyLoss(nn.Module):
    def __init__(self, apply_nonlin=None, batch_dice=False, do_bg=True, smooth=1., alpha=0.3, beta=0.7):
        """
        """
...
class TV_and_CE_loss(nn.Module):
    def __init__(self, tversky_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1,
        ...
        :param weight_dice:
        """
        super(TV_and_CE_loss, self).__init__()
        ...
        if not square_dice:
            self.dc = TverskyLoss(apply_nonlin=softmax_helper, **tversky_kwargs)
        else:
            self.dc = TverskyLoss(apply_nonlin=softmax_helper, **tversky_kwargs)
