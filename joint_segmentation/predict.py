...
from nnunet.inference.segmentation_export import save_segmentation_nifti_from_softmax_2dplus3d


def preprocess_save_to_queue(preprocess_fn_2d, preprocess_fn_3d, q, list_of_lists, output_files, segs_from_prev_stage, classes,
                             transpose_forward):
    # suppress output
    ...
    for i, l in enumerate(list_of_lists):
        try:
            ...
            d_2d, _, dct_2d = preprocess_fn_2d(l)
            d_3d, _, dct_3d = preprocess_fn_3d(l)
            ...
            if segs_from_prev_stage[i] is not None:
                ...
                seg_reshaped = resize_segmentation(seg_prev, d_3d.shape[1:], order=1)
                seg_reshaped = to_one_hot(seg_reshaped, classes)
                d_3d = np.vstack((d_3d, seg_reshaped)).astype(np.float32)
            ...
            print(d_2d.shape)
            print(d_3d.shape)
            if np.prod(d_2d.shape) > (2e9 / 4 * 0.85):  # *0.85 just to be save, 4 because float32 is 4 bytes
                print(
                    "This output is too large for python process-process communication. "
                    "Saving output temporarily to disk")
                np.save(output_file[:-7] + "_2d.npy", d_2d)
                d_2d = output_file[:-7] + "_2d.npy"
            
            if np.prod(d_3d.shape) > (2e9 / 4 * 0.85):  # *0.85 just to be save, 4 because float32 is 4 bytes
                print(
                    "This output is too large for python process-process communication. "
                    "Saving output temporarily to disk")
                np.save(output_file[:-7] + "_3d.npy", d_3d)
                d_3d = output_file[:-7] + "_3d.npy"


            q.put((output_file, (d_2d, dct_2d), (d_3d, dct_3d)))
...
def preprocess_multithreaded(trainer_2d, trainer_3d, list_of_lists, output_files, num_processes=2, segs_from_prev_stage=None):
    if segs_from_prev_stage is None:
        segs_from_prev_stage = [None] * len(list_of_lists)

    num_processes = min(len(list_of_lists), num_processes)

    classes = list(range(1, trainer_2d.num_classes))
    assert isinstance(trainer_2d, nnUNetTrainer)
    assert isinstance(trainer_3d, nnUNetTrainer)
    q = Queue(1)
    processes = []
    for i in range(num_processes):
        pr = Process(target=preprocess_save_to_queue, args=(trainer_2d.preprocess_patient,
                                                            trainer_3d.preprocess_patient, q,
                                                            list_of_lists[i::num_processes],
                                                            output_files[i::num_processes],
                                                            segs_from_prev_stage[i::num_processes],
                                                            classes, trainer_2d.plans['transpose_forward']))
        pr.start()
        processes.append(pr)
...
def predict_cases_2dplus3d(model_2d, model_3d, list_of_lists, output_filenames, folds_2d, folds_3d, save_npz, num_threads_preprocessing,
    ...
    print("loading parameters for folds_2d,", folds_2d)
    print("loading parameters for folds_3d,", folds_3d)
    trainer_2d, params_2d = load_model_and_checkpoint_files(model_2d, folds_2d, mixed_precision=mixed_precision,
                                                      checkpoint_name=checkpoint_name)

    trainer_3d, params_3d = load_model_and_checkpoint_files(model_3d, folds_3d, mixed_precision=mixed_precision,
                                                      checkpoint_name=checkpoint_name)

    if segmentation_export_kwargs is None:
        if 'segmentation_export_params' in trainer_3d.plans.keys():
            force_separate_z = trainer_3d.plans['segmentation_export_params']['force_separate_z']
            interpolation_order = trainer_3d.plans['segmentation_export_params']['interpolation_order']
            interpolation_order_z = trainer_3d.plans['segmentation_export_params']['interpolation_order_z']
        else:
    ...
    preprocessing = preprocess_multithreaded(trainer_2d, trainer_3d, list_of_lists, cleaned_output_files, num_threads_preprocessing,
                                             segs_from_prev_stage)
    print("starting prediction...")
    all_output_files = []
    for preprocessed in preprocessing:
        output_filename, (d_2d, dct_2d), (d_3d, dct_3d) = preprocessed
        all_output_files.append(all_output_files)
        if isinstance(d_2d, str):
            data_2d = np.load(d_2d)
            os.remove(d_2d)
            d_2d = data_2d

        if isinstance(d_3d, str):
            data_3d = np.load(d_3d)
            os.remove(d_3d)
            d_3d = data_3d

        print("predicting", output_filename)
        trainer_2d.load_checkpoint_ram(params_2d[0], False)
        trainer_3d.load_checkpoint_ram(params_3d[0], False)

        print('In predict_cases_2dplus_3d, trainer_2d, 3d, ', trainer_2d, trainer_3d)
        


        softmax_2d = trainer_2d.predict_preprocessed_data_return_seg_and_softmax(
            d_2d, do_mirroring=do_tta, mirror_axes=trainer_2d.data_aug_params['mirror_axes'], use_sliding_window=True,
            step_size=step_size, use_gaussian=True, all_in_gpu=all_in_gpu,
            mixed_precision=mixed_precision)[1]

        for p_2d in params_2d[1:]:
            trainer_2d.load_checkpoint_ram(p_2d, False)
        ...
        if len(params_2d) > 1:
            softmax_2d /= len(params_2d)


        softmax_3d = trainer_3d.predict_preprocessed_data_return_seg_and_softmax(
            d_3d, do_mirroring=do_tta, mirror_axes=trainer_3d.data_aug_params['mirror_axes'], use_sliding_window=True,
            step_size=step_size, use_gaussian=True, all_in_gpu=all_in_gpu,
            mixed_precision=mixed_precision)[1]


        for p_3d in params_3d[1:]:
            trainer_3d.load_checkpoint_ram(p_3d, False)
            softmax_3d += trainer_3d.predict_preprocessed_data_return_seg_and_softmax(
                d_3d, do_mirroring=do_tta, mirror_axes=trainer_3d.data_aug_params['mirror_axes'], use_sliding_window=True,
                step_size=step_size, use_gaussian=True, all_in_gpu=all_in_gpu,
                mixed_precision=mixed_precision)[1]
        ...
        if len(params_3d) > 1:
            softmax_3d /= len(params_3d)

        transpose_forward = trainer_2d.plans.get('transpose_forward')

        print('2d, 3d, transpose forward, ', trainer_2d.plans.get('transpose_forward'),trainer_3d.plans.get('transpose_forward'))
        assert(trainer_2d.plans.get('transpose_forward')==trainer_3d.plans.get('transpose_forward'))
        if transpose_forward is not None:
            transpose_backward = trainer_2d.plans.get('transpose_backward')
            softmax_2d = softmax_2d.transpose([0] + [i + 1 for i in transpose_backward])

            softmax_3d = softmax_3d.transpose([0] + [i + 1 for i in transpose_backward])
        ...
        if hasattr(trainer_3d, 'regions_class_order'):
            region_class_order = trainer_3d.regions_class_order
            region_class_order_2d = trainer_2d.regions_class_order
        else:
            region_class_order = None
            region_class_order_2d = None
        print('region_class order 2d, 3d ', region_class_order_2d, region_class_order)
        ...
        if np.prod(softmax_2d.shape) > (2e9 / bytes_per_voxel * 0.85):  # * 0.85 just to be save
            print(
                "This output is too large for python process-process communication. Saving output temporarily to disk")
            np.save(output_filename[:-7] + "_2d.npy", softmax_2d)
            softmax_2d = output_filename[:-7] + "_2d.npy"

        
        if np.prod(softmax_3d.shape) > (2e9 / bytes_per_voxel * 0.85):  # * 0.85 just to be save
            print(
                "This output is too large for python process-process communication. Saving output temporarily to disk")
            np.save(output_filename[:-7] + "_3d.npy", softmax_3d)
            softmax_3d = output_filename[:-7] + "_3d.npy"

        results.append(pool.starmap_async(save_segmentation_nifti_from_softmax_2dplus3d,
                                          ((softmax_2d, softmax_3d, output_filename, dct_2d, dct_3d, interpolation_order, region_class_order,
                                            None, None,
...
def predict_cases_arc(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
                  num_threads_nifti_save, segs_from_prev_stage=None, do_tta=True, mixed_precision=True,
                  overwrite_existing=False,
                  all_in_gpu=False, step_size=0.5, checkpoint_name="model_final_checkpoint",
                  segmentation_export_kwargs: dict = None, disable_postprocessing: bool = False):
...
def predict_from_folder(model_2d: str, model_3d: str, input_folder: str, output_folder: str, folds_2d: Union[Tuple[int], List[int]],
                        folds_3d: Union[Tuple[int], List[int]],
    ...
    # model_2d, model_3d-> ckpt folder name
    shutil.copy(join(model_2d, 'plans.pkl'), output_folder)

    assert isfile(join(model_2d, "plans.pkl")), "Folder with saved model weights must contain a plans.pkl file"
    assert isfile(join(model_3d, "plans.pkl")), "Folder with saved model weights must contain a plans.pkl file"
    expected_num_modalities = load_pickle(join(model_2d, "plans.pkl"))['num_modalities']
    ...
    if mode == "normal":
        ...
        return predict_cases_2dplus3d(model_2d, model_3d, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], 
                             folds_2d, folds_3d,
                             save_npz, num_threads_preprocessing, num_threads_nifti_save, lowres_segmentations, tta,
                             mixed_precision=mixed_precision, overwrite_existing=overwrite_existing,
                             all_in_gpu=all_in_gpu,
